
package model.bean;


public class Visitante {
    
    private String Nome_Visitante, CPF_Visitante, Telefone_Visitante;
    private int id_Visitante;

    /**
     * @return the Nome_Visitante
     */
    public String getNome_Visitante() {
        return Nome_Visitante;
    }

    /**
     * @param Nome_Visitante the Nome_Visitante to set
     */
    public void setNome_Visitante(String Nome_Visitante) {
        this.Nome_Visitante = Nome_Visitante;
    }

    /**
     * @return the CPF_Visitante
     */
    public String getCPF_Visitante() {
        return CPF_Visitante;
    }

    /**
     * @param CPF_Visitante the CPF_Visitante to set
     */
    public void setCPF_Visitante(String CPF_Visitante) {
        this.CPF_Visitante = CPF_Visitante;
    }

    /**
     * @return the Telefone_Visitante
     */
    public String getTelefone_Visitante() {
        return Telefone_Visitante;
    }

    /**
     * @param Telefone_Visitante the Telefone_Visitante to set
     */
    public void setTelefone_Visitante(String Telefone_Visitante) {
        this.Telefone_Visitante = Telefone_Visitante;
    }

    /**
     * @return the id_Visitante
     */
    public int getId_Visitante() {
        return id_Visitante;
    }

    /**
     * @param id_Visitante the id_Visitante to set
     */
    public void setId_Visitante(int id_Visitante) {
        this.id_Visitante = id_Visitante;
    }
    
}
